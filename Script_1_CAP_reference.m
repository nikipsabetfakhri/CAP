...
for ds = 1:n_datasets
    ...
    n_subjects{ds} = nSubj;               % subjects in this dataset
    TC{ds} = cell(1, n_subjects{ds});     % allocate inner cell for subjects
    ...
    for s = 1:n_subjects{ds}
        ...
        % --- Load functional volumes (match GUI prefix) ---
        ...
        V = spm_vol(files);
        Y = spm_read_vols(V);                  % [X Y Z T]
        Y2D = reshape(Y, [], size(Y,4))';      % [nTP × nVox]

        % --- Apply mask for this dataset ---
        tmp_data = Y2D(:, mask{ds});           % [nTP × nMaskedVox]
        nTP = size(tmp_data,1);

        % --- Z-score per voxel across time (GUI behavior) ---
        tmp_data = zscore(tmp_data, 0, 1);
        ...
        if isempty(motfile)
            ...
            thisFD = CAP_ComputeFD(fullfile(subj_dir, motfile(1).name)); % [nTP × 1]
        ...
        % Build FD{ds} matrix column by column
        if s == 1
            FD{ds} = zeros(nTP, n_subjects{ds}); % allocate once we know nTP
        end
        FD{ds}(:, s) = thisFD;
    ...
    % --- Store dataset-wide sizes (GUI only sets this once, on first dataset) ---
    if ds == 1
        SubjSize.TP  = size(TC{ds}{1}, 1);
        SubjSize.VOX = size(TC{ds}{1}, 2);
    end

    % --- Resample underlay to functional resolution (GUI does this only once) ---
    if ds == 1
...
for ds = 1:n_datasets
    ...
    % Store outputs
    RetainedPct{ds} = p_ds(3,:);   % % of frames retained per subject
    FrameIdx{ds}    = Indices_ds;  % struct with kept/scrubbed indices
...
Xon_ref = horzcat(Xonp{ReferencePopulation}{:})';   % [frames × voxels]
...
% Frame retention violin plots across groups

% Concat frame-retention percentages across datasets
...
%         Consensus = zeros(size(Ck,1), size(Ck,2), numel(K_range));
%     end
