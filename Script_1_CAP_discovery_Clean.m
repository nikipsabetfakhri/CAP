...
for s = 1:nSubj
    ...
    if isempty(motfile)
        ...
        FD{s} = CAP_ComputeFD(motfile_name);   % <-- use provided function
        if length(FD{s}) ~= nTP
            error('FD length (%d) ≠ number of timepoints (%d) for subject %s', ...
...
% Convert FD to matrix form [nTP × nSubj] if all same length
if numel(unique(cellfun(@length,FD))) == 1
    FD = cell2mat(FD);  % overwrite FD with numeric matrix
else
    error('Subjects have unequal FD lengths – check preprocessing!');
end

% Convert FD to matrix form [nTP × nSubj] if all same length
if numel(unique(cellfun(@length,FD))) == 1
    FDmat = cell2mat(FD);  % [nTP × nSubj]
else
    warning('Subjects have unequal FD lengths — keep as cell array.');
    FDmat = FD;  % keep cell if lengths differ
end
...
% --- Sanity check: are dimensions consistent? ---
[IsOK, Problems] = CAP_IsDataOK(TC, FD, mask, brain_info);
if ~IsOK
...
% --- Z-score CAP maps (voxelwise) ---
CAPs_z = CAP_Zscore(CAPs);  % CAPs = [K × voxels]


save(fullfile('/home/nsabet/TbCAPs/SavedData', ...
    sprintf('PreConsensus_%s.mat', datestr(now,'yyyymmdd_HHMM'))), ...
    'TC','FD','mask','brain_info','Indices','Xon','Tmot','ref_img','-v7.3');
